...
const GRID_CC={
  neutral:{bg:"rgba(99,102,241,0.07)",bd:"rgba(99,102,241,0.18)",tx:"#94a3b8",bx:"#c7d2fe",hbg:"rgba(99,102,241,0.28)",hbd:"rgba(99,102,241,0.55)"},
};
...
function GrowthReferenceGrid({records,baby,birthDate,sex}){
  ...
  return(
    <div style={{padding:"4px 0"}}>
      ...
      {/* Legend */}
      <div style={{display:"flex",gap:12,flexWrap:"wrap",marginBottom:12,fontSize:10,alignItems:"center"}}>
          <div style={{display:"flex",alignItems:"center",gap:4}}>
            <div style={{width:9,height:9,borderRadius:2,background:"rgba(99,102,241,0.07)",border:"1px solid rgba(99,102,241,0.18)"}}/>
            <span style={{color:"#64748b"}}>WHO reference value</span>
          </div>
          <div style={{display:"flex",alignItems:"center",gap:4}}>
            <div style={{width:9,height:9,borderRadius:2,background:"rgba(99,102,241,0.28)",border:"1px solid rgba(99,102,241,0.55)"}}/>
            <span style={{color:"#64748b"}}>logged measurement intersects this row</span>
          </div>
          <div style={{display:"flex",alignItems:"center",gap:5}}>
            <span style={{fontSize:9,fontWeight:700,color:"#e2e8f0"}}>BOLD</span>
            <span style={{color:"#64748b"}}>= data logged this week</span>
          </div>
        </div>
            ...
            <tr>
              ...
              {GRID_WEEKS.map(w=>(
                <th key={w} style={{padding:"2px 0",fontSize:6,textAlign:"center",
                  ...
                  color:weekInfoMap[w]?"#e2e8f0":"#64748b"}}>
                  {w===1||w%4===0?`W${w}`:"·"}
                </th>
              ))}
            </tr>
                  ...
                  {entryMode==="monthly" ? (
                    ...
                    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:8}}>
                      <div>
                        ...
                        <input style={iStyle} type="number" min="0" max="365" value={customDays}
                          onChange={e=>{
                            setCustomDays(e.target.value);
                            const d=parseInt(e.target.value);
                            if(!isNaN(d)&&d>=0&&birthDate){
                              const dt=new Date(birthDate);dt.setDate(dt.getDate()+d);
                              setCustomDate(dt.toISOString().split("T")[0]);
                            } else setCustomDate("");
                          }} placeholder="e.g. 42"/>
                        {customDays&&<div style={{fontSize:11,color:"#64748b",marginTop:3}}>≈ {(parseInt(customDays)/30.4375).toFixed(1)} months</div>}
                      </div>
                      <div>
                        <label style={lStyle}>Date of measurement</label>
                        <input style={iStyle} type="date" value={customDate}
                          onChange={e=>{
                            setCustomDate(e.target.value);
                            if(e.target.value&&birthDate){
                              const diff=Math.round((new Date(e.target.value)-new Date(birthDate))/86400000);
                              if(diff>=0&&diff<=365) setCustomDays(String(diff));
                            }
                          }} max={new Date().toISOString().split("T")[0]}/>
                        {customDate&&customDays&&<div style={{fontSize:11,color:"#64748b",marginTop:3}}>= Day {customDays} from birth</div>}
                      </div>
                    </div>
                    <div style={{marginBottom:14}}>
